...
## 2026-09-07 상품 사진 정리·상품정보 검토

- 분석 실행 버튼은 사진 목록 하단. 구간별 기준 사진 선택/사진별 번역 입력 UI 제거.
- merchandising.py의 한국어 작문 보조 프롬프트로 내부 번역 → 핵심 장점·상품정보를 연결. 추정 사용감은 검토 참고에 표시하며 수치·인증·제조국 등은 창작하지 않음.
- 모델이 품목에 맞는 product_info 항목을 추천. 사용자 추가·수정·삭제 가능, 빈 값은 최종 표에서 생략.
- v4 사진 선택은 사진별 분석·구간 계획·장점 적합도 순위 사용. 공통 대표 사진 강제 삽입 제거, CHECK POINT 첫 사진 중복 완화.
- CHECK POINT/DETAIL은 글자·그래픽이 있는 사진에 최소 편집을 요청한 뒤 조판. 색상 편집과 정리는 함께 처리하며 이미지·색상·품질·용도별 캐시 사용. 깨끗하고 색상이 일치하는 원본은 편집 호출 없이 사용.
- PRODUCT INFO는 제목 → 흰 배경 제품 누끼컷 → 판매 색상 안내 → 확정 상품정보 표. 문구는 Pillow 조판. 원본이 착용 사진이면 모델이 사람을 제거하므로 제품 디테일 최종 확인 필요.
- 기존 시안 파일을 자동 변경하지 않음. 새 분석에서 추천 항목/새 작문 적용. 기존 v4 시안은 문구·상품정보 저장 후 해당 구간 재생성 시 새 조판 적용.
- 추가 배포 파일 merchandising.py 필요. 새 규칙이 prompt_v3.txt를 대체하는 것이 아니라 런타임 최우선 보조 프롬프트로 덧붙음.

- 구간별 문구: 확정 핵심장점은 2번 요약에서 그대로 사용. HERO는 2~3단어, 개별 CHECK POINT는 각 장점을 쉬운 사용 장면으로 확장한 별도 문구 사용. 확정 문구·분석 근거·모델이 바뀌면 파생 문구 캐시 갱신. 고화질과 재생성은 동일한 파생 문구 재사용. 기존 결과 파일은 해당 구간 재생성 시 변경.


## 2026-09-07 구도·색상 검사 및 검토 화면 정리

- 문구 확인은 선택해서 분석한 사진만 표시하며 확대창 이동도 같은 목록 사용. 생성 결과/고화질/다운로드를 별도 카드로 분리하고 문구 검토 단계에서는 숨김.
- HERO 보조문구는 36자 이하 감성 한 줄. 개별 CHECK POINT 설명은 글꼴 실제 폭으로 3줄 초과 응답을 재작성. 요약의 사용자 확정 문구는 유지하면서 아이콘·카드·배경색으로 구성.
- PRODUCT INFO 상단 컬러 띠 제거. 컬러는 정보표에서만 표시.
- 자동 product_bbox 크롭 제거(사용자 명시 crop만 유지). 입력 사진을 비율 유지해 가장 가까운 API 화면 비율에 패딩, 확대·줌인 금지. 제품이 거의 안 보이는 자료는 생성 참조에서 제외.
- 판매 색상을 포함한 복수 색상 원본도 대표 색상으로 정리. 원본 그대로 사용하는 경우에도 판매 색상 검사.
- image_review.py: GPT로 원본/결과의 제품 구조·판매 색상·구도·글자 잔존을 비교. 실패한 편집은 원본에서 최대 한 번 재생성하며 통과 결과만 캐시. 새 장면은 포즈 변경 허용, 제품 구조/색상 유지 검사. 모델 검사로 완벽한 일치를 보장하지는 않음.
- 안전 검사 거절은 한글 안내로 표시하고 자동 반복하지 않음. 성인 모델의 일반적인 스포츠·일상 광고 맥락을 명시하며 신체 강조 대신 제품 전체와 사용 상황을 요청.
- 배포 추가 파일 image_review.py 필수. 사진 및 파생 문구 캐시 버전 갱신. 기존 시안은 자동 재생성하지 않음.
